...
final class CloudDefaults: NSObject {
    ...
    private var canSync: Bool {
        ...
        logger.info("\(#function, privacy: .public): iCloud syncing \(token == nil ? "disabled" : "enabled", privacy: .public)")
        return token != nil
    }
    ...
    private func start() {
        ...
        setupDone = true
        logger.debug("\(#function, privacy: .public): synchronized successfully")
    }
    ...
    func synchronize() {
        logger.debug("\(#function, privacy: .public)")
        guard canSync else {
            logger.debug("\(#function, privacy: .public): cannot sync")
            return
        }
        if !setupDone {
            logger.debug("\(#function, privacy: .public): setup not done")
            start()
        } else {
            logger.debug("\(#function, privacy: .public): synching")
            #if DEBUG
            ...
            if ubiquitousKeyValueStore.synchronize() {
                logger.debug("\(#function, privacy: .public): success")
            } else {
                logger.debug("\(#function, privacy: .public): error")
            }
        }
    }
    ...
    private func findNewSyncKeys(_ notification: Notification) {
        logger.debug("\(#function, privacy: .public): \(notification.name.rawValue, privacy: .public)")
        for (key, value) in userDefaults.dictionaryRepresentation() {
            guard key.hasPrefix(Self.syncPrefix) && synchronizedUserDefaultKeys.contains(key) == false else { continue }

            logger.debug("\(#function, privacy: .public): New key found \(key, privacy: .public), adding observer and pushing value to cloud store")
    ...
    private func updateLocal(_ notification: Notification) {
        logger.debug("\(#function, privacy: .public): \(notification.debugDescription, privacy: .public)")
        guard let userInfo = notification.userInfo else { return }
        guard let reasonForChange = userInfo[NSUbiquitousKeyValueStoreChangeReasonKey] as? Int else { return }

        switch reasonForChange {
        case NSUbiquitousKeyValueStoreServerChange:
            logger.info("\(#function, privacy: .public): Server change")
        case NSUbiquitousKeyValueStoreInitialSyncChange:
            logger.info("\(#function, privacy: .public): Initial Sync change")
        case NSUbiquitousKeyValueStoreQuotaViolationChange:
            logger.info("\(#function, privacy: .public): Quota Violation change")
        case NSUbiquitousKeyValueStoreAccountChange:
            logger.info("\(#function, privacy: .public): Account change")
        default:
        ...
        for (key, value) in ubiquitousKeyValueStore.dictionaryRepresentation {
            ...
            logger.debug("\(#function, privacy: .public): Updating local value of \(key, privacy: .public)")
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        guard ignoreLocalChanges == false else {
            logger.debug("\(#function, privacy: .public): UserDefaults \(keyPath?.description ?? "-", privacy: .public) changed, but ignored")
            return
        }
        guard let keyPath = keyPath,
              let newValue = change?[.newKey]
        else {
            logger.debug("\(#function, privacy: .public): UserDefaults \(keyPath?.description ?? "-", privacy: .public) but invalid keyPath or value provided")
            return
        }

        logger.debug("\(#function, privacy: .public): UserDefaults \(keyPath, privacy: .public) changed to \(String(describing: newValue))")
